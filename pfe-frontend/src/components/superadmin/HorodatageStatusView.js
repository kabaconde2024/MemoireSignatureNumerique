...
const HorodatageStatusView = ({ setSnackbar, isMobile = false, isTablet = false }) => {
    ...
    const fetchStatus = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://memoiresignaturenumerique.onrender.com/api/horodatage/statut', { withCredentials: true });
    ...
    const testerHorodatage = async () => {
        setTesting(true);
        try {
            const response = await axios.post('https://memoiresignaturenumerique.onrender.com/api/horodatage/tester', { data: testData || "Test automatique" }, { withCredentials: true });
