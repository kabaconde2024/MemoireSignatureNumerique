...
const ConfigurationView = ({ setSnackbar, isMobile = false, isTablet = false }) => {
    const [config, setConfig] = useState({
        ...
        otpLongueur: 6,
        mfaCodeExpirationMinutes: 1  // ✅ Ajout de la configuration MFA
    });
    ...
    const fetchConfig = async () => {
        try {
            // ✅ Utilisation de l'instance API (gère déjà le cookie et le baseURL)
                ...
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: mobile ? 2 : 3, borderRadius: '16px' }}>
                        ...
                        <TextField 
                            ...
                            sx={{ mb: 3 }} 
                            size={mobile ? "small" : "medium"} 
                            helperText="📅 Nombre de jours avant expiration du lien de signature" 
                            InputProps={{ endAdornment: <Typography variant="caption" color="textSecondary">jours</Typography> }} 
                        />
                        <Divider sx={{ my: 2 }} />
